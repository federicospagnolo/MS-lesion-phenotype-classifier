...
class PatchesFromCSV(Dataset):
    def __init__(
        ...
        if label_id is not None:
            ...
            #), f"The specified label_id column: {label_id} is not present in the CSV"
        ...
        self.imgs_dir = os.path.dirname(self.csv)
        ...
        if self.label_id:
            drawn = {
                **{
                    ...
                    BATCHKEYS.LABEL: (
                         1 if self.task == "multiclass" and self.df[self.label_id][index] in {1, 7} else
                         2 if self.task == "multiclass" and self.df[self.label_id][index] in {2, 8} else
                         1 if self.task != "multiclass" and self.df[self.label_id][index] in {1, 7} else
                         0
                         )
                },

...
class BasicRimNetInferer:
    ...
    def test_end(self, outputs: List[dict]):
        ...
        if self.csv_summary is None:
            self.create_summary(
                ['F1', 'AUC', 'tp_t', 'fp_t', 'fn_t', 'tn_t']
            )
            
        self.append_metrics_to_summary(
            [mean_f1_dice, overall_auc, tp, fp, fn, tn]
        )        
...
class BasicRimNetXAI:
    def __init__(
        self, test_folder, results_path, checkpoint_path, model: ModularRimNet, device=torch.device("cuda")
    ):
        assert isinstance(model, ModularRimNet)
    ...
    def __call__(
        ...
        for batch in tqdm(dataloader):
                ...
                flair_layers = [self.model.subnet_for_modality[1][1][0].conv_block[0], self.model.subnet_for_modality[1][1][0].conv_block[3], self.model.subnet_for_modality[1][1][0].conv_block[3]]
                phase_layers = [self.model.subnet_for_modality[2][1][0].conv_block[0], self.model.subnet_for_modality[2][1][0].conv_block[3], self.model.subnet_for_modality[2][1][0].conv_block[3]]
                ...
                tag = f'{subject}_{lesion}_FLAIR.nii.gz'
                ...
                gcam_fl = nib.Nifti1Image(np.pad(flair_gcam, pad_width, mode="constant", constant_values=0), input_affine)
                gcam_ph = nib.Nifti1Image(np.pad(phase_gcam, pad_width, mode="constant", constant_values=0), input_affine)
                ...
                ig_flair = nib.Nifti1Image(np.pad(integ[1], pad_width, mode="constant", constant_values=0), input_affine)
                ig_phase = nib.Nifti1Image(np.pad(integ[2], pad_width, mode="constant", constant_values=0), input_affine)
                ...
                #nib.save(g_ph, self.results_path + "/" + group + "/grads_ph_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                #nib.save(g_fl, self.results_path + "/" + group + "/grads_fl_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                #nib.save(flair, self.results_path + "/" + group + "/flair_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                #nib.save(phase, self.results_path + "/" + group + "/phase_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                #nib.save(gcam, self.results_path + "/" + group + "/GCAM_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                nib.save(gcam_fl, self.results_path + "/" + group + "/GCAM_fl_" + subject +
                "_Lesion_" + lesion + ".nii.gz")
                nib.save(gcam_ph, self.results_path + "/" + group + "/GCAM_ph_" + subject +
                "_Lesion_" + lesion + ".nii.gz")
                #nib.save(baseline, self.results_path + "/" + group + "/baseline_" + subject +
                #"_Lesion_" + lesion + ".nii.gz")
                nib.save(ig_flair, self.results_path + "/" + group + "/IG_fl_" + subject +
                "_Lesion_" + lesion + ".nii.gz")
                nib.save(ig_phase, self.results_path + "/" + group + "/IG_ph_" + subject +
                "_Lesion_" + lesion + ".nii.gz")
               
        pd.concat(predictions_df).to_csv(self.results_path + '/results.csv')        

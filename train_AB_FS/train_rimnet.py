...
class ContrastBasedTransforms(MapTransform):
    ...
    def __call__(self, data):
        ...
        data["img"][1] = self.transform(data["img"][1]) # t1
        data["img"][2] = self.transform(data["img"][2]) # qsm
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 5:
        ...
        modalities = [f"QSM_{third_contrast}", f"cont_{base_contrast}", f"cont_{ph_contrast}"]
    ...
    class Args:
        ...
        task = "multiclass"
        if mask_contrast == "":
             experiment_out = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/training/{task}/{folder}/{sub_folder}/{ph_contrast}_{base_contrast}'
        else:
             experiment_out = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/training/{task}/{folder}/{sub_folder}/{ph_contrast}_{base_contrast}_{mask_contrast}'
        if base_contrast == 'FLAIR' or base_contrast == 'T1':
             csv_path_train = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/INsIDER_MS_patches_nested_kfold_{task}/{folder}/{sub_folder}/train/train_data.csv'
             csv_path_val = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/INsIDER_MS_patches_nested_kfold_{task}/{folder}/{sub_folder}/val/val_data.csv'
        else:      
             csv_path_train = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/INsIDER_MS_patches_nested_kfold_{task}/{folder}/{sub_folder}/train/train_data.csv'
             csv_path_val = f'/home/federico.spagnolo/Federico/PRLsConfluent/train_AB_FS/INsIDER_MS_patches_nested_kfold_{task}/{folder}/{sub_folder}/val/val_data.csv'
    ...
    if args.task == "multiclass":
           ...
           model = TrimodalRimNet(input_size, batchnorm=True).to(device)       
    else:
